...
namespace DicingBlade.Classes
{
    ...
    {
        public event Action<object> OnSettingsChangedEvent;

        public void SetChangedObject(object obj)
        {
            OnSettingsChangedEvent?.Invoke(obj);
        }
    }
}
...
namespace DicingBlade.ViewModels
{
    ...
    internal class MainViewModel : IDisposable
    {
        ...
        [NotNull] private readonly ExceptionsAgregator _exceptionsAgregator;
        private bool _homeDone;
        [NotNull] private readonly IMachine _machine;
        private ITechnology _technology;
        [NotNull] private readonly SettingsChangeService _settingsChangeService = new();
        ...
        public MainViewModel()
        {
            ...
            try
            {
                ...
                _machine.ConfigureSensors(new Dictionary<Sensors, (Ax, Di)>
                {
                    ...
                    {Sensors.SpindleCoolant, (Ax.Y, Di.In2)}
                });
            ...
            _settingsChangeService.OnSettingsChangedEvent += _settingsChangeService_OnSettingsChangedEvent;
            AjustWaferTechnology();
        }

        private void _settingsChangeService_OnSettingsChangedEvent(object obj)
        {
            if (obj is IWafer)
            {
                var wf = (IWafer)obj;
        ...
        private async Task ToTeachCutShift()
        {
                ...
                MessageBoxResult.OK)
            {
                ...
                if (Process is not null) Process.TeachDiskShift();
        ...
        private async Task KeyDownAsync(object args)
        {
            ...
            if (key.Key == Key.Multiply) UserConfirmation = true;
            if (key.Key == Key.Q)
            {
                ...
                if (_homeDone)
                {
                    if (Process == null)
                    {
                        try
                        {
                            ...
                            _settingsChangeService.OnSettingsChangedEvent += Process.SubstrateChanged;
                        }
                        catch (Exception ex)
                        {
        ...
        private void WaferSettings()
        {
            ...
            {
                DataContext = new WaferSettingsViewModel(_settingsChangeService)
            };
        ...
        private void TechnologySettings()
        {
            ...
            _technology = new Technology().DeSerializeObjectJson(Settings.Default.TechnologyLastFile);
            Process?.RefreshTechnology(_technology);
            Wafer?.SetPassCount(PropContainer.Technology.PassCount);
        }
